...
require.cache[musicPlayerPath] = {
    ...
    loaded: true,
    exports: {
        prepareAllPlayersForShutdown() { activeEvents?.push('snapshot'); },
...
test('shutdown 冪等，並先凍結音樂快照再取消 HTTP 與外部程序', async () => {
    const events = [];
    const manifests = [
...
test('任一 shutdown phase 失敗仍會完成後續清理並回報 aggregate error', async () => {
    ...
    assert.deepEqual(events, [
        'snapshot', 'abort', 'scheduler', 'process', 'stop:feature', 'players', 'client'
    ]);
    assert.equal(harness.client.destroyed, true);
});
