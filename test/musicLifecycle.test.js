...
test('音樂面板、序列、暫停、移除、清空與 preparation 狀態皆走公開 handler', async t => {
    ...
    assert.equal(buttonRows[0].components[1].data.disabled, true);
    assert.equal(command._test.createButtons({ current: null, queue: [{}] })[0].components[1].data.disabled, true);
    assert.equal(command._test.createButtons({ current: {}, queue: [] })[0].components[1].data.disabled, true);
    assert.equal(command._test.createButtons({ current: {}, queue: [{}] })[0].components[1].data.disabled, false);
    const liveState = {
        current: {
