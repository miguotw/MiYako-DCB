...
test('loadConfig 回傳統一 camelCase 結構，空白外部憑證可停用輪詢', () => {
    ...
    try {
        ...
        assert.equal(config.commands.gameCheckIn.timezone, 8);
...
test('固定數值範圍與 Discord 欄位限制會被驗證', () => {
    const cases = [
        ...
        ['configCommands.yml', document => { document.gameCheckIn.timezone = -13; }],
        ['configCommands.yml', document => { document.gameCheckIn.timezone = 15; }],
