...
- `music.allowLiveStreams` 控制直播功能；全域同時直播數由 `music.maxConcurrentLiveStreams` 限制，額滿時依 FIFO 等待。來源中斷會在 `music.liveReconnectWindowSeconds` 內以 1／2／4／8／20 秒退避重新解析。
- 直播暫停會關閉上游程序並釋放 slot；繼續時重新解析並接回當下 live edge。
